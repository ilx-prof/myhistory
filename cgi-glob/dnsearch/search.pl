...
# Определение глобальных переменных.
our $action = exists $in{ 'action' } ? $in{ 'action' } : "";
our $search = exists $in{ 'search' } ? $in{ 'search' } : "";
our $dir    = exists $in{ 'dir'    } ? $in{ 'dir'    } : "";

# Разбор путей.
our ( $dir_url, $dir_path );
if ( $dir ) {
...
our $logic       = $in{ 'logic'       } ? $in{ 'logic'       } : "and";
our $register    = $in{ 'register'    } ? $in{ 'register'    } : "i";
our $mode        = $in{ 'mode'        } ? $in{ 'mode'        } : "usual";
our $current_str = $in{ 'current_str' } ? $in{ 'current_str' } : 0;
our $in_find     = $in{ 'in_find'     } ? $in{ 'in_find'     } : 0;
...
our ( $form, $header, $footer, $search_header, $search_center, $search_footer, $e1, $e2, $help ) = getTemplates();
