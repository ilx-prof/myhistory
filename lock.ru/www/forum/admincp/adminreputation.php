...
define('CVS_REVISION', '$RCSfile: adminreputation.php,v $ - $Revision: 1.82 $');
...
if ($_POST['do'] == 'update')
{
	...
	$vbulletin->GPC['reputationlevel']['level'] = htmlspecialchars_uni($vbulletin->GPC['reputationlevel']['level']);
	...
	if (!$db->query_first("SELECT reputationlevelid FROM " . TABLE_PREFIX . "reputationlevel WHERE minimumreputation = " . $vbulletin->GPC['reputationlevel']['minimumreputation'] . $sql))
	{
		...
		{ // edit
			...
			print_stop_message('saved_reputation_level_x_successfully', $vbulletin->GPC['reputationlevel']['level']);
		}
		else
		{
...
if ($_POST['do'] == 'kill')
{
	...
	$db->query_write("DELETE FROM " . TABLE_PREFIX . "reputationlevel WHERE minimumreputation=" . $vbulletin->GPC['minimumreputation']);
...
if ($_REQUEST['do'] == 'dolist')
{
	...
	print_form_header('adminreputation', 'dolist');
	print_table_header(construct_phrase($vbphrase['x_reputation_comments_page_y_z'], vb_number_format($totalrep), $vbulletin->GPC['pagenumber'], vb_number_format($totalpages)), 6);
	...
	while ($comment = $db->fetch_array($comments))
	{
		...
		$cell[] = iif ($comment['reason'], '<span class="smallfont">' . construct_link_code(htmlspecialchars_uni($comment['reason']), "../showthread.php?" . $vbulletin->session->vars['sessionurl'] . "postid=$comment[postid]#$comment[postid]", 1) . '</span>');
		$cell[] = construct_link_code($vbphrase['edit'], "adminreputation.php?" . $vbulletin->session->vars['sessionurl'] . "do=editreputation&reputationid=$comment[reputationid]") .
			' ' . construct_link_code($vbphrase['delete'], "adminreputation.php?" . $vbulletin->session->vars['sessionurl'] . "do=deletereputation&reputationid=$comment[reputationid]");
...
|| ####################################################################
|| #        SCRiPTMAFiA 2005 - THE DiRTY HANDS ON YOUR SCRiPTS
|| # CVS: $RCSfile: adminreputation.php,v $ - $Revision: 1.82 $
|| ####################################################################
\*======================================================================*/
?>
