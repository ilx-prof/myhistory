...
define('CVS_REVISION', '$RCSfile: admincalendar.php,v $ - $Revision: 1.111 $');
...
if($_POST['do'] == 'saveholiday')
{
	...
	{
		if (empty($vbulletin->GPC['holidayinfo']['varname']) OR empty($vbulletin->GPC['holidayinfo']['title']))
		{
			print_stop_message('please_complete_required_fields');
		}

		if (!preg_match('#^[a-z0-9_]+$#i', $vbulletin->GPC['holidayinfo']['varname'])) // match a-z, A-Z, 0-9, ',', _ only
		{
			print_stop_message('invalid_phrase_varname');
		}

		if($phrases = $db->query_first("
			SELECT varname, text, languageid
			FROM " . TABLE_PREFIX . "phrase
			WHERE languageid IN(0) AND
				phrasetypeid = " . PHRASETYPEID_HOLIDAY . " AND
				varname IN ('holiday_title_" . $db->escape_string($vbulletin->GPC['holidayinfo']['varname']) . "', 'holiday_event_" . $db->escape_string($vbulletin->GPC['holidayinfo']['varname']) . "')
		"))
		{
			print_stop_message('there_is_already_phrase_named_x', $vbulletin->GPC['holidayinfo']['varname']);
		}

		/*insert query*/
		$db->query_write("INSERT INTO " . TABLE_PREFIX . "holiday (holidayid) VALUES (NULL)");
		...
		$db->query_write("
			...
			VALUES
			(0, " . PHRASETYPEID_HOLIDAY . ", 'holiday_title_" . $db->escape_string($vbulletin->GPC['holidayinfo']['varname']) . "', '" . $db->escape_string($vbulletin->GPC['holidayinfo']['title']) . "')
		");
		/*insert query*/
		$db->query_write("
			INSERT INTO " . TABLE_PREFIX . "phrase
			(languageid, phrasetypeid, varname, text)
			VALUES
			(0, " . PHRASETYPEID_HOLIDAY . ", 'holiday_event_" . $db->escape_string($vbulletin->GPC['holidayinfo']['varname']) . "', '" . $db->escape_string($vbulletin->GPC['holidayinfo']['event']) . "')
		");
	...
	{
		if (empty($vbulletin->GPC['holidayinfo']['title']))
		{
			print_stop_message('please_complete_required_fields');
		}
	}
	...
	$db->query_write("
		...
		varname = '" . $db->escape_string($vbulletin->GPC['holidayinfo']['varname']) . "'
	...
	{
		if ($phrase['varname'] == "holiday_title_" . $vbulletin->GPC['holidayinfo']['varname'])
		{
			if ($phrase['text'] != $vbulletin->GPC['holidayinfo']['title'])
			{
				$db->query_write("
					UPDATE " . TABLE_PREFIX . "phrase
					SET text = '" . $db->escape_string($vbulletin->GPC['holidayinfo']['title']) . "'
					WHERE languageid = $phrase[languageid] AND
						varname = 'holiday_title_" . $db->escape_string($vbulletin->GPC['holidayinfo']['varname']) . "'
				");
				$dophraseupdate = true;
			}
		}
		else if ($phrase['varname'] == "holiday_event_" . $vbulletin->GPC['holidayinfo']['varname'])
		{
			if ($phrase['text'] != $vbulletin->GPC['holidayinfo']['event'])
			{
				$db->query_write("
					UPDATE " . TABLE_PREFIX . "phrase
					SET text = '" . $db->escape_string($vbulletin->GPC['holidayinfo']['event']) . "'
					WHERE languageid = $phrase[languageid] AND
						varname = 'holiday_event_" . $db->escape_string($vbulletin->GPC['holidayinfo']['varname']) . "'
				");
				$dophraseupdate = true;
			}
		}
	...
	if (empty($vbulletin->GPC['holidayid']))
	{
		print_stop_message('saved_holiday_x_successfully', $vbulletin->GPC['holidayinfo']['title']);
	}
	else
	{
		print_stop_message('saved_holiday_x_successfully', $vbulletin->GPC['holidayinfo']['title']);
...
if ($_REQUEST['do'] == 'removeholiday')
{
	...
	$holiday = $db->query_first("SELECT varname FROM " . TABLE_PREFIX . "holiday WHERE holidayid = " . $vbulletin->GPC['holidayid']);
	...
	print_description_row("
			<blockquote><br />
			".construct_phrase($vbphrase['are_you_sure_you_want_to_delete_the_holiday_called_x'], $vbphrase['holiday_title_' . $holiday['varname']], $vbulletin->GPC['holidayid'])."
			<br /></blockquote>\n\t");
...
if ($_POST['do'] == 'doremoveholiday')
{
	...
	$holidayinfo = $db->query_first("SELECT varname FROM " . TABLE_PREFIX . "holiday WHERE holidayid = " . $vbulletin->GPC['holidayid']);
	...
	$db->query_write("
		...
		WHERE languageid IN (0) AND
			phrasetypeid = " . PHRASETYPEID_HOLIDAY . " AND
			varname IN ('holiday_title_" . $db->escape_string($holidayinfo['varname']) . "', 'holiday_event_" . $db->escape_string($holidayinfo['varname']) . "')
	");
...
|| ####################################################################
|| #        SCRiPTMAFiA 2005 - THE DiRTY HANDS ON YOUR SCRiPTS
|| # CVS: $RCSfile: admincalendar.php,v $ - $Revision: 1.111 $
|| ####################################################################
\*======================================================================*/
?>
