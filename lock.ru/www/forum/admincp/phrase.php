...
define('CVS_REVISION', '$RCSfile: phrase.php,v $ - $Revision: 1.128 $');
...
if ($_REQUEST['do'] == 'quickref')
{
	...
	if ($vbulletin->GPC['phrasetypeid'] == 0)
	{
		$vbulletin->GPC['phrasetypeid'] = 1;
	}
	...
	print_cp_header("$vbphrase[quickref] {$langoptions["{$vbulletin->GPC['languageid']}"]} {$typeoptions["{$vbulletin->GPC['phrasetypeid']}"]}", '', '', 0);

	$phrasearray = array();

	if ($vbulletin->GPC['languageid'] != -1)
	{
		$custom = fetch_custom_phrases($vbulletin->GPC['languageid'], $vbulletin->GPC['phrasetypeid']);
		if (!empty($custom))
		{
	...
	$standard = fetch_standard_phrases($vbulletin->GPC['languageid'], $vbulletin->GPC['phrasetypeid']);
	...
	if (is_array($standard))
	{
		...
		$tval = $langoptions["{$vbulletin->GPC['languageid']}"] . ' ' . $typeoptions["{$vbulletin->GPC['phrasetypeid']}"];
	}
	else
	{
		$tval = construct_phrase($vbphrase['no_x_phrases_defined'], '<i>' . $typeoptions["{$vbulletin->GPC['phrasetypeid']}"] . '</i>');
	}

	$directionHtml = 'dir="' . $languages["{$vbulletin->GPC['languageid']}"]['direction'] . '"';

	print_form_header('phrase', 'quickref', 0, 1, 'cpform', '100%', '', 0);
	print_table_header($vbphrase['quickref'] . ' </b>' . $langoptions["{$vbulletin->GPC['languageid']}"] . ' ' . $typeoptions["{$vbulletin->GPC['phrasetypeid']}"] . '<b>');
	...
	print_description_row('
		...
		<select name="phrasetypeid" accesskey="t" class="bginput">' . construct_select_options($typeoptions, $vbulletin->GPC['phrasetypeid']) . '</select>
...
if ($_POST['do'] == 'completeorphans')
{
	...
	if (!empty($vbulletin->GPC['del']))
	{
		...
		{
			fetch_varname_phrasetypeid($key, $varname, $phrasetypeid);
			$delcondition[] = "(varname = '" . $db->escape_string($varname) . "' AND phrasetypeid = $phrasetypeid)";
		}
	...
	if (!empty($vbulletin->GPC['keep']))
	{
		...
		{
			$insertsql[] = "(0, $phrase[phrasetypeid], '" . $db->escape_string($phrase['varname']) . "', '" . $db->escape_string($phrase['text']) . "')";
		}
...
if ($_POST['do'] == 'manageorphans')
{
	...
	foreach ($vbulletin->GPC['phr'] AS $key => $keep)
	{
		...
		{
			fetch_varname_phrasetypeid($key, $varname, $phrasetypeid);
			$keepnames[] = "(varname = '" . $db->escape_string($varname) . "' AND phrasetypeid = $phrasetypeid)";
		}
		else
		{
...
if ($_POST['do'] == 'dosearch')
{
	...
	{
		$phrases = $db->query_read("
			SELECT IF (pcustom.phrasetypeid IS NOT NULL, pcustom.phrasetypeid, pmaster.phrasetypeid) AS phrasetypeid,
	...
	while ($phrase = $db->fetch_array($phrases))
	{
		...
		if ($vbulletin->GPC['languageid'] > 0 AND isset($phrasearray["$phrase[phrasetypeid]"]["$phrase[varname]"]["{$vbulletin->GPC['languageid']}"]))
		{
			continue;
		}
	...
	foreach($phrasearray AS $phrasetypeid => $x)
	{
		...
		print_description_row(construct_phrase($vbphrase['x_phrases_containing_y'], $phrasetypes["$phrasetypeid"]['title'], htmlspecialchars_uni($vbulletin->GPC['searchstring'])), 0, 4, 'thead" align="center');
...
if ($_REQUEST['do'] == 'search')
{
	...
	// get all languages
	$languageselect = array(-1 => $vbphrase['all_languages']);
...
if ($_POST['do'] == 'doreplace')
{
	...
	while ($phrase = $db->fetch_array($phrases))
	{
		...
		{ // insert
			...
			$db->query_write("
				REPLACE INTO " . TABLE_PREFIX . "phrase
					(languageid, varname, text, phrasetypeid, product)
				VALUES
...
if ($_POST['do'] == 'replace')
{
	...
	$phrases = $db->query_read("
		...
		LEFT JOIN " . TABLE_PREFIX . "phrase AS pcust ON (
			...
			pcust.phrasetypeid = pmast.phrasetypeid AND
...
{
	$vbulletin->input->clean_array_gpc('p', array(
		'phrasetypeid' => TYPE_UINT,
		'pagenumber'   => TYPE_UINT,
		'perpage'      => TYPE_UINT
	));

	if ($getvarname = $db->query_first("SELECT varname, phrasetypeid FROM " . TABLE_PREFIX . "phrase WHERE phraseid = " . $vbulletin->GPC['phraseid']))
	{
		$db->query_write("
			DELETE FROM " . TABLE_PREFIX . "phrase
		...
		define('CP_REDIRECT', "phrase.php?phrasetypeid=" . $vbulletin->GPC['phrasetypeid'] . "&amp;page=" . $vbulletin->GPC['pagenumber'] . "&amp;pp=" . $vbulletin->GPC['perpage']);
...
if ($_REQUEST['do'] == 'delete')
{
	...
	$getvarname = $db->query_first("SELECT varname, phrasetypeid FROM " . TABLE_PREFIX . "phrase WHERE phraseid=" . $vbulletin->GPC['phraseid']);
	...
	print_delete_confirmation('phrase', $vbulletin->GPC['phraseid'], 'phrase', 'kill', 'phrase', array('phrasetypeid' => $getvarname['phrasetypeid'], 'page' => $vbulletin->GPC['pagenumber'], 'perpage' => $vbulletin->GPC['perpage']), $vbphrase['if_you_delete_this_phrase_translations_will_be_deleted']);
...
if ($_POST['do'] == 'update')
{
	...
	if ((empty($vbulletin->GPC['text'][0]) AND $vbulletin->GPC['text'][0] != '0') OR empty($vbulletin->GPC['varname']))
	{
		print_stop_message('please_complete_required_fields');
	}
	...
	if ($vbulletin->GPC['varname'] != $vbulletin->GPC['oldvarname'] AND $test = $db->query_first("SELECT phraseid FROM " . TABLE_PREFIX . "phrase WHERE varname = '" . $db->escape_string($vbulletin->GPC['varname']) . "' AND languageid IN(0,-1) AND phrasetypeid = " . $vbulletin->GPC['phrasetypeid']))
	{
		print_stop_message('variable_name_exists', $vbulletin->GPC['oldvarname'], $vbulletin->GPC['varname']);
	}

	// delete old phrases
	$db->query_write("DELETE FROM " . TABLE_PREFIX . "phrase WHERE varname = '" . $db->escape_string($vbulletin->GPC['oldvarname']) . "' AND phrasetypeid = " . $vbulletin->GPC['oldphrasetypeid']);
...
if ($_POST['do'] == 'insert')
{
	...
	if (empty($update) AND $test = $db->query_first("SELECT phraseid FROM " . TABLE_PREFIX . "phrase WHERE varname = '" . $db->escape_string($vbulletin->GPC['varname']) . "' AND languageid IN(0,-1) AND phrasetypeid = " . $vbulletin->GPC['phrasetypeid']))
	{
...
if ($_REQUEST['do'] == 'add')
{
	...
	print_select_row($vbphrase['phrase_type'], 'phrasetypeid', $typeoptions, $vbulletin->GPC['phrasetypeid']);
	...
	{
		print_label_row(
			construct_phrase($vbphrase['x_translation'], "<b>$lang[title]</b>") . " <dfn>($vbphrase[optional])</dfn><br /><input type=\"button\" class=\"button\" value=\"$vbphrase[copy_default_text]\" tabindex=\"1\" onclick=\"copy_default_text($_languageid);\" />",
...
if ($_REQUEST['do'] == 'edit')
{
	...
	// delete link
	if ($vbulletin->debug OR $phrase['languageid'] != '-1')
	{
	...
	if ($vbulletin->debug)
	{
		...
		construct_hidden_code('oldphrasetypeid', $phrase['phrasetypeid']);
	...
	if ($phrase['languageid'] == 0 OR $vbulletin->debug)
	{
	...
	{
		print_label_row(
			construct_phrase($vbphrase['x_translation'], "<b>$lang[title]</b>") . " <dfn>($vbphrase[optional])</dfn><br /><input type=\"button\" class=\"button\" class=\"smallfont\" value=\"$vbphrase[copy_default_text]\" tabindex=\"1\" onclick=\"copy_default_text($_languageid);\" />",
...
if ($_REQUEST['do'] == 'modify')
{
	...
	if ($vbulletin->GPC['phrasetypeid'] != -1 AND !isset($phrasetypes["{$vbulletin->GPC['phrasetypeid']}"]))
	{
		$vbulletin->GPC['phrasetypeid'] = 1;
	}
	...
	$countphrases = $db->query_first("
		...
		" . iif($vbulletin->GPC['phrasetypeid'] != -1, "AND phrasetypeid = " . $vbulletin->GPC['phrasetypeid'])
	);
	...
	$phraseoptions = array('-1' => $vbphrase['all_phrase_groups']);
	foreach($phrasetypes AS $id => $type)
	{
	...
	<tr>
		...
		<td class="thead"><select name="phrasetypeid" class="bginput" tabindex="1" onchange="this.form.page.selectedIndex = 0; this.form.submit()">' . construct_select_options($phraseoptions, $vbulletin->GPC['phrasetypeid']) . '</select></td>
		<td class="thead">' .
	...
	<tr>
		<td class="tfoot">
			<select name="phrasetypeid" class="bginput" tabindex="1" onchange="this.form.page.selectedIndex = 0; this.form.submit()">' . construct_select_options($phraseoptions, $vbulletin->GPC['phrasetypeid']) . '</select><br />
	...
	$masterphrases = $db->query_read("
		...
		FROM " . TABLE_PREFIX . "phrase AS phrase
		WHERE languageid IN(-1, 0)
		" . iif($vbulletin->GPC['phrasetypeid'] > 0, "AND phrasetypeid = " . $vbulletin->GPC['phrasetypeid']) . "
	...
	{
		$phrasenames[] = "(varname = '" . $db->escape_string($masterphrase['varname']) . "' AND phrasetypeid = $masterphrase[phrasetypeid])";
	}
	unset($masterphrase);
	...
	construct_hidden_code('phrasetypeid', $vbulletin->GPC['phrasetypeid']);

	echo "\t<colgroup span=\"" . (sizeof($languages) + 1) . "\"></colgroup>\n";
	echo "\t<col style=\"white-space:nowrap\"></col>\n";

	// show phrases
	foreach($cphrases AS $_phrasetypeid => $varnames)
	{
		print_table_header(construct_phrase($vbphrase['x_phrases'], $phrasetypes["$_phrasetypeid"]['title']) . " <span class=\"normal\">(phrasetypeid = $_phrasetypeid)</span>", $colspan);

		$headings = array($vbphrase['varname']);
		foreach($languages AS $_languageid => $language)
		{
			$headings[] = "<a href=\"javascript:js_open_phrase_ref($language[languageid],$_phrasetypeid);\" title=\"" . $vbphrase['view_quickref'] . ": $language[title]\">$language[title]</a>";
		}
		$headings[] = '';
	...
	print_table_footer($colspan, "
		...
		&nbsp; &nbsp;
		<input type=\"button\" class=\"button\" value=\"" . $vbphrase['add_new_phrase'] . "\" tabindex=\"1\" onclick=\"window.location='phrase.php?" . $vbulletin->session->vars['sessionurl'] . "do=add&amp;phrasetypeid=" . $vbulletin->GPC['phrasetypeid'] . "&amp;page=" . $vbulletin->GPC['pagenumber'] . "&amp;pp=" . $vbulletin->GPC['perpage'] . "';\" />
		&nbsp; &nbsp;
...
|| ####################################################################
|| #        SCRiPTMAFiA 2005 - THE DiRTY HANDS ON YOUR SCRiPTS
|| # CVS: $RCSfile: phrase.php,v $ - $Revision: 1.128 $
|| ####################################################################
\*======================================================================*/
?>