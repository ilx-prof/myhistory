...
define('CVS_REVISION', '$RCSfile: image.php,v $ - $Revision: 1.96 $');
...
// ###################### Start displayitem #######################
function print_image_item($item, $itemid, $itempath, $page, $perpage, $catid, $massmove = false)
{
...
if ($_POST['do'] == 'insertcategory')
{
	...
	$db->query_write("INSERT INTO " . TABLE_PREFIX . "imagecategory (
		...
		NULL, '" . $db->escape_string($vbulletin->GPC['title']) . "', $catid, " . $vbulletin->GPC['displayorder'] . "
	)");
...
if ($_POST['do'] == 'doupload')
{
	...
	if (is_uploaded_file($vbulletin->GPC['imagefile']['tmp_name']))
	{
		if (empty($vbulletin->GPC['title'])
			OR empty($vbulletin->GPC['imagespath'])
			OR ($vbulletin->GPC['table'] == 'smilie' AND empty($vbulletin->GPC['smilietext']))
			)
		{
			print_stop_message('please_complete_required_fields');
		}
		if (file_exists("./" . $vbulletin->GPC['imagespath'] . "/" . $vbulletin->GPC['imagefile']['name']))
		{
			print_stop_message('file_x_already_exists', htmlspecialchars_uni($imagefile['name']));
		}

		$result = copy($vbulletin->GPC['imagefile']['tmp_name'], "./" . $vbulletin->GPC['imagespath'] . "/" . $vbulletin->GPC['imagefile']['name']);
		if (!$result)
		{
			$error = ob_get_contents();
			print_stop_message('error_writing_x', htmlspecialchars_uni($vbulletin->GPC['imagefile']['name']));
		}
		$vbulletin->GPC['imagespath'] .= '/' . $vbulletin->GPC['imagefile']['name'];
...
if ($_POST['do'] == 'insert')
{
	...
	if ($vbulletin->GPC['table'] == 'smilie' AND $db->query_first("SELECT smilieid FROM " . TABLE_PREFIX . $vbulletin->GPC['table'] . " WHERE BINARY smilietext = '" . $db->escape_string($vbulletin->GPC['smilietext']) . "'"))
	{
		...
		{ // if the image is being uploaded zap it
			unlink($vbulletin->GPC['imagespath']);
		}
	...
	$db->query_write("
		...
		(
			...
			. "'" . $db->escape_string($vbulletin->GPC['imagespath']) . "',"
			. $vbulletin->GPC['imagecategoryid'] . ","
...
|| ####################################################################
|| #        SCRiPTMAFiA 2005 - THE DiRTY HANDS ON YOUR SCRiPTS
|| # CVS: $RCSfile: image.php,v $ - $Revision: 1.96 $
|| ####################################################################
\*======================================================================*/
?>
