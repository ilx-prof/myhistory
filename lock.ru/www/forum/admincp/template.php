...
define('CVS_REVISION', '$RCSfile: template.php,v $ - $Revision: 1.232 $');
...
if ($_REQUEST['do'] == 'findupdates')
{
	...
	foreach($stylecache AS $styleid => $style)
	{
		if (is_array($customcache["$styleid"]))
		{
			...
			{
				$globaldate = vbdate($vbulletin->options['dateformat'], $template['globaldate']);
				$globaltime = vbdate($vbulletin->options['timeformat'], $template['globaldate']);
				if ($template['customdate'])
				{
					$customdate = vbdate($vbulletin->options['dateformat'], $template['customdate']);
					$customtime = vbdate($vbulletin->options['timeformat'], $template['customdate']);
				}
				else
				{
...
if ($_REQUEST['do'] == 'download')
{
	...
	if (function_exists('set_time_limit') AND (ini_get('safe_mode') == 1 OR strtolower(ini_get('safe_mode')) == 'on'))
	{
		@set_time_limit(1200);
	}
	...
	$xml = new XMLexporter();
	$xml->add_group('style', array('name' => $vbulletin->GPC['title'], 'vbversion' => $vbulletin->options['templateversion'], 'product' => $vbulletin->GPC['product'], 'type' => iif($vbulletin->GPC['dostyleid'] == -1, 'master', 'custom')));
...
if ($_POST['do'] == 'killstyle')
{
	...
	if ($check['numstyles'] <= 1)
	{
		...
		$db->query_write("DELETE FROM " . TABLE_PREFIX . "style");
...
if ($_POST['do'] == 'updatetemplate')
{
	...
	");
	if ($vbulletin->GPC['title'] != $old_template['title'] AND $db->query_first("
		SELECT templateid
		FROM " . TABLE_PREFIX . "template
		WHERE styleid = $old_template[styleid] AND title = '" . $db->escape_string($vbulletin->GPC['title']) . "'
	"))
	{
		echo '<pre>'; print_r($old_template); echo '</pre>';
...
if ($_POST['do'] == 'kill')
{
	...
	$template = $db->query_first("SELECT styleid FROM " . TABLE_PREFIX . "template WHERE templateid = " . $vbulletin->GPC['templateid']);
...
|| ####################################################################
|| #        SCRiPTMAFiA 2005 - THE DiRTY HANDS ON YOUR SCRiPTS
|| # CVS: $RCSfile: template.php,v $ - $Revision: 1.232 $
|| ####################################################################
\*======================================================================*/
?>