...
define('CVS_REVISION', '$RCSfile: thread.php,v $ - $Revision: 1.100 $');
define('NOZIP', 1);
...
if ($_POST['do'] == 'doremovepoll')
{
	...
	$thread = $db->query_first("SELECT threadid,pollid FROM " . TABLE_PREFIX . "thread WHERE threadid = " . $vbulletin->GPC['threadid'] . " AND pollid=" . $vbulletin->GPC['pollid']);
	if ($thread)
	{
...
if ($_POST['do'] == 'dospecificunsubscribe')
{
	...
	{
		$threadids = preg_replace('#\s+#', ',', $vbulletin->GPC['ids']);
...
if ($_REQUEST['do'] == 'confirmunsubscribe')
{
	...
	$vbulletin->input->clean_array_gpc('r', array(
		'threadids' => TYPE_NOHTML,
	));

	//echo "<pre>[$threadids]</pre>\n";
	if (!isset($threadids))
	{
		$threadids = $vbulletin->GPC['threadids'];
	}
...
|| ####################################################################
|| #        SCRiPTMAFiA 2005 - THE DiRTY HANDS ON YOUR SCRiPTS
|| # CVS: $RCSfile: thread.php,v $ - $Revision: 1.100 $
|| ####################################################################
\*======================================================================*/
?>
