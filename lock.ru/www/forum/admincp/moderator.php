...
define('CVS_REVISION', '$RCSfile: moderator.php,v $ - $Revision: 1.97 $');
...
if ($_POST['do'] == 'update')
{
	...
	print_stop_message('saved_moderator_x_successfully', htmlspecialchars_uni($vbulletin->GPC['modusername']));
...
if ($_REQUEST['do'] == "showlist")
{
	...
	while ($moderator = $db->fetch_array($moderators))
	{
		if ($curmod != $moderator['userid'])
		{
			...
			echo "\n\t<ul>\n\t<li><b><a href=\"user.php?" . $vbulletin->session->vars['sessionurl'] . "do=edit&u=$moderator[userid]\">$moderator[username]</a></b><span class=\"smallfont\"> - " . $vbphrase['last_online'] . " <span class=\"$onlinecolor\">" . $lastonline . "</span></span>\n";
		...
		echo "\t\t\t<li><a href=\"../forumdisplay.php?" . $vbulletin->session->vars['sessionurl'] . "f=$moderator[forumid]\" target=\"_blank\">$moderator[title]</a>\n".
			"\t\t\t\t<span class=\"smallfont\">(" . construct_link_code($vbphrase['edit'], "moderator.php?" . $vbulletin->session->vars['sessionurl'] . "do=edit&moderatorid=$moderator[moderatorid]").
...
if ($_REQUEST['do'] == "showmods")
{
	...
	while ($forum = $db->fetch_array($forums))
	{
		...
		echo "\t\t\t<li><a href=\"user.php?" . $vbulletin->session->vars['sessionurl'] . "do=edit&amp;u=$forum[userid]\" target=\"_blank\">$forum[username]</a>" .
			"\t\t\t\t<span class=\"smallfont\">(" . construct_link_code($vbphrase['edit'], "moderator.php?" . $vbulletin->session->vars['sessionurl'] . "do=edit&moderatorid=$forum[moderatorid]") .
...
if ($_POST['do'] == 'killall')
{
	...
	{
		$db->query_write("DELETE FROM " . TABLE_PREFIX . "moderator WHERE userid = " . $vbulletin->GPC['userid']);
...
|| ####################################################################
|| #        SCRiPTMAFiA 2005 - THE DiRTY HANDS ON YOUR SCRiPTS
|| # CVS: $RCSfile: moderator.php,v $ - $Revision: 1.97 $
|| ####################################################################
\*======================================================================*/
?>