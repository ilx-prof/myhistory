...
define('CVS_REVISION', '$RCSfile: attachment.php,v $ - $Revision: 1.172 $');
...
if ($_REQUEST['do'] == 'domoveattachment')
{
	...
	while ($attachment = $db->fetch_array($attachments))
	{
		...
		{ // Converting FROM mysql TO fs
			...
			if (@filesize(fetch_attachment_path($attachment['userid'], $attachment['attachmentid'])) == 0)
			{
	...
	else
	{
		...
		if ($vbulletin->options['attachfile'] == ATTACH_AS_DB)
		{
			...
			print_description_row(construct_phrase($vbphrase['attachment_removal'],
													$totalattach['count'],
													$vbulletin->GPC['count'],
													$vbulletin->GPC['attacherrorcount']
								));
...
if ($_POST['do'] == 'confirmfileremove')
{
	function rmpath($dir)
	{
		...
		if ($handle = opendir($dir))
		{
			...
			if (preg_match('#^[0-9]+$#i', basename($dir)) AND $dir != $vbulletin->options['attachpath'])
			{
				closedir($handle);
				if (@rmdir($dir))
				{
	...
	rmpath($vbulletin->options['attachpath']);
...
if ($_POST['do'] == 'doedit')
{
	...
	if (!empty($vbulletin->GPC['upload']['tmp_name']) OR !empty($vbulletin->GPC['url']))
	{
		...
		$upload->userinfo = array('userid' => $attachment['userid']);
...
if ($_REQUEST['do'] == 'types')
{
	...
	print_table_header($vbphrase['edit_attachment_types'], 8);
	print_cells_row(array(
		$vbphrase['extension'],
	...
	print_submit_row($vbphrase['add_new_attachment_type'], 0, 8);
}
...
|| ####################################################################
|| #        SCRiPTMAFiA 2005 - THE DiRTY HANDS ON YOUR SCRiPTS
|| # CVS: $RCSfile: attachment.php,v $ - $Revision: 1.172 $
|| ####################################################################
\*======================================================================*/
?>
