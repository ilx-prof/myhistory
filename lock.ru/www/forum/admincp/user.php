...
define('CVS_REVISION', '$RCSfile: user.php,v $ - $Revision: 1.335 $');
...
if ($_REQUEST['do'] == 'find')
{
	...
	$searchquery = "
		...
		parentemail,(options & " . $vbulletin->bf_misc_useroptions['coppauser'] . ") AS coppauser, homepage, icq, aim, yahoo, msn, signature,
...
if ($_REQUEST['do'] == 'edit' OR $_REQUEST['do'] == 'add')
{
	...
	if ($vbulletin->GPC['userid'])
	{
		...
		$user = $db->query_first("
			...
			NOT ISNULL(customavatar.filedata) AS hascustomavatar, usertextfield.signature,
		...
		{
			$quicklinks[
				"../register.php?" . $vbulletin->session->vars['sessionurl'] . "do=requestemail&email=" . urlencode(unhtmlspecialchars($user['email']))
		...
		$quicklinks = array_merge(
			...
			"usertools.php?" . $vbulletin->session->vars['sessionurl'] . "do=doips&u=" . $vbulletin->GPC['userid']
...
if ($_POST['do'] == 'update')
{
	...
	$userdata->set_usertitle(
		...
		$vbulletin->usergroupcache[$vbulletin->GPC['user']['usergroupid']],
		true,
...
if ($_POST['do'] == 'updateaccess')
{
	...
	$db->query("DELETE FROM " . TABLE_PREFIX . "access WHERE userid = " . $vbulletin->GPC['userid']);
...
if ($_REQUEST['do'] == 'modify')
{
	...
	$profilefields = $db->query_read("SELECT profilefieldid, title FROM " . TABLE_PREFIX . "profilefield");
	while ($profilefield = $db->fetch_array($profilefields))
	{
...
if ($_REQUEST['do'] == 'find2' AND defined('DONEFIND'))
{
	...
	$profilefields = $db->query_read("SELECT profilefieldid,title,type,data FROM " . TABLE_PREFIX . "profilefield");
	while ($profilefield = $db->fetch_array($profilefields))
	{
...
if ($_REQUEST['do'] == 'moderate')
{
	...
	else
	{
		...
		while ($user = $db->fetch_array($users))
		{
			...
			$cell[] = "<a href=\"mailto:$user[email]\">$user[email]</a>";
			$cell[] = "<a href=\"usertools.php?" . $vbulletin->session->vars['sessionurl'] . "do=doips&amp;depth=2&amp;ipaddress=$user[ipaddress]\" target=\"_user\">$user[ipaddress]</a>";
			$cell[] = "
...
|| ####################################################################
|| #        SCRiPTMAFiA 2005 - THE DiRTY HANDS ON YOUR SCRiPTS
|| # CVS: $RCSfile: user.php,v $ - $Revision: 1.335 $
|| ####################################################################
\*======================================================================*/
?>
