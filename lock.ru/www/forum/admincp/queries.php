...
define('CVS_REVISION', '$RCSfile: queries.php,v $ - $Revision: 1.52 $');
...
if ($_POST['do'] == 'doquery')
{
	...
	print_form_header('', '');
	print_table_header($vbphrase['query'] . iif($vbulletin->GPC['autoquery'] > 0, ' (' . $queryoptions["{$vbphrase['all_users']}"]["{$vbulletin->GPC['autoquery']}"] . ')'));
	...
	switch ($querytype)
	{
		...
		case 'DROP':
			...
			else
			{
				...
				{
					print_description_row(construct_phrase($vbphrase['affected_rows'], vb_number_format($db->affected_rows())));
				}
				print_table_footer();
			}
			break;
		...
		default:
			...
			else
			{
				...
				print_table_header("$vbphrase[results]: " . vb_number_format($numrows) . ', ' . construct_phrase($vbphrase['page_x_of_y'], $vbulletin->GPC['pagenumber'], $numpages), $colcount);
				if ($numrows)
				{
					$collist = array();
...
|| ####################################################################
|| #        SCRiPTMAFiA 2005 - THE DiRTY HANDS ON YOUR SCRiPTS
|| # CVS: $RCSfile: queries.php,v $ - $Revision: 1.52 $
|| ####################################################################
\*======================================================================*/
?>