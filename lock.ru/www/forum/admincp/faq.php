...
define('CVS_REVISION', '$RCSfile: faq.php,v $ - $Revision: 1.72 $');
...
if ($_POST['do'] == 'insert')
{
	$vars = array(
		...
		'title'			=> TYPE_ARRAY_STR,	// Origionaly NULL though not type checking incode, used as an array
		'text'			=> TYPE_ARRAY_STR,	// Origionaly NULL though not type checking incode, used as an array
		'deftitle'		=> TYPE_STR,
		'deftext'		=> TYPE_STR
	);
	...
	if ($check = $db->query_first("SELECT varname FROM " . TABLE_PREFIX . "phrase WHERE varname = '" . $db->escape_string($vbulletin->GPC['faq']) . "' AND phrasetypeid IN (" . PHRASETYPEID_FAQTITLE . "," . PHRASETYPEID_FAQTEXT . ")"))
	{
	...
	foreach(array_keys($vbulletin->GPC['title']) AS $languageid)
	{
		...
		{
			$insertSql[] = "($languageid, '$faqname', '" . $db->escape_string($newtitle) . "', " . PHRASETYPEID_FAQTITLE . ")";
			$insertSql[] = "($languageid, '$faqname', '" . $db->escape_string($newtext) . "', " . PHRASETYPEID_FAQTEXT . ")";
		}
	}
...
if ($_REQUEST['do'] == 'edit' OR $_REQUEST['do'] == 'add')
{
	...
	else
	{
		...
		print_input_row($vbphrase['varname'], 'faq', 'new_faq_item', 0, '35" onblur="js_check_shortname(this.form, ' . iif($vbulletin->debug, 'false', 'true') . ');');
	}
...
|| ####################################################################
|| #        SCRiPTMAFiA 2005 - THE DiRTY HANDS ON YOUR SCRiPTS
|| # CVS: $RCSfile: faq.php,v $ - $Revision: 1.72 $
|| ####################################################################
\*======================================================================*/
?>
