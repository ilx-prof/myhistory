...
define('CVS_REVISION', '$RCSfile: language.php,v $ - $Revision: 1.134 $');
...
if ($_POST['do'] == 'download')
{
	...
	$getphrases = $db->query_read("
		...
		WHERE (languageid = " . $vbulletin->GPC['dolanguageid'] . " OR languageid = 0)
...
if ($_POST['do'] == 'update')
{
	...
	if (!empty($vbulletin->GPC['rvt']))
	{
		...
		$query = "
			...
			### DELETE REVERTED PHRASES FROM LANGUAGE:" . $vbulletin->GPC['dolanguageid'] . ", PHRASETYPE:" . $vbulletin->GPC['phrasetypeid'] . " ###
		";
	...
	foreach (array_keys($vbulletin->GPC['def']) AS $varname)
	{
		...
		{
			$sql[] = "(" . $vbulletin->GPC['dolanguageid'] . ", " . $vbulletin->GPC['phrasetypeid'] . ", '" . $db->escape_string($varname) . "', '" . $db->escape_string($newphrase) . "', '" . $db->escape_string($vbulletin->GPC['product']) . "')";
		}
	}
	...
	if (!empty($sql))
	{
		...
		$query = "
			### UPDATE CHANGED PHRASES FROM LANGUAGE:" . $vbulletin->GPC['dolanguageid'] . ", PHRASETYPE:" . $vbulletin->GPC['phrasetypeid'] . " ###
	...
	define('CP_REDIRECT', "language.php?do=edit&amp;dolanguageid=" . $vbulletin->GPC['dolanguageid'] . "&amp;phrasetypeid=" . $vbulletin->GPC['phrasetypeid'] . "&amp;page=" . $vbulletin->GPC['pagenumber']);
...
if ($_REQUEST['do'] == 'edit')
{
	...
	if ($vbulletin->GPC['phrasetypeid'] < 1)
	{
	...
	{
		print_cp_redirect("phrase.php?" . $vbulletin->session->vars['sessionurl'] . "phrasetypeid=" . $vbulletin->GPC['phrasetypeid'], 0);
	}
	...
	if ($vbulletin->GPC['dolanguageid'] != -1)
	{
		$custom_phrases = fetch_custom_phrases($vbulletin->GPC['dolanguageid'], $vbulletin->GPC['phrasetypeid']);
		$numcustom = sizeof($custom_phrases);
	}
	// get inherited and customized phrases
	$standard_phrases = fetch_standard_phrases($vbulletin->GPC['dolanguageid'], $vbulletin->GPC['phrasetypeid'], $numcustom);
	...
	echo '
		...
		<tr>
			...
			<td class="thead" rowspan="2">' . "
				<input type=\"button\" class=\"button\" value=\"" . $vbphrase['view_quickref'] . "\" onclick=\"js_open_phrase_ref({$vbulletin->GPC['dolanguageid']}, $vbulletin->GPC['phrasetypeid']);\" />
		...
		<tr>
			...
			<td class="thead"><select name="phrasetypeid" onchange="this.form.page.selectedIndex = 0; this.form.submit()" class="bginput">' . construct_select_options($phrasetypeoptions, $vbulletin->GPC['phrasetypeid']) . '</select></td>
		</tr>
	';
	print_table_footer();
	...
	print_table_header(construct_phrase($vbphrase['edit_translate_x_y_phrases'], $languages["{$vbulletin->GPC['dolanguageid']}"]['title'], "<span class=\"normal\">" . $phrasetypes["{$vbulletin->GPC['phrasetypeid']}"][title] . "</span>") . ' <span class="normal">' . construct_phrase($vbphrase['page_x_of_y'], $vbulletin->GPC['pagenumber'], $numpages) . '</span>');
	...
	foreach ($printers AS $key => $blarg)
	{
		...
		print_phrase_row($blarg, $phrasetypes["{$vbulletin->GPC['phrasetypeid']}"]['editrows'], $key, $language['direction']);
...
|| ####################################################################
|| #        SCRiPTMAFiA 2005 - THE DiRTY HANDS ON YOUR SCRiPTS
|| # CVS: $RCSfile: language.php,v $ - $Revision: 1.134 $
|| ####################################################################
\*======================================================================*/
?>