...
if ($_POST['do'] == 'update')
{
	...
	$eventdata->setr_info('parseurl', $vbulletin->GPC['parseurl']);
	...
	{ // No Eventid == Insert Event
		...
		if ($calendarinfo['neweventemail'])
		{
			...
			$eventmessage = strip_bbcode($vbulletin->GPC['message']);
...
if ($_REQUEST['do'] == 'viewreminder')
{
	...
	$eventcount = $db->query_first("
		...
		WHERE subscribeevent.userid = " . $vbulletin->userinfo['userid'] . "
			AND event.visible = 1
		" . (!empty($calendarid) ? "AND event.calendarid = $calendarid" : '') . "
	");
	...
	if ($totalevents = $db->num_rows($getevents))
	{
		...
		while ($event = $db->fetch_array($getevents))
		{
			...
			$offset = $event['utc'] ? $vbulletin->userinfo['timezoneoffset'] : ($vbulletin->userinfo['timezoneoffset'] ? $vbulletin->userinfo['tzoffset'] : $vbulletin->userinfo['timezoneoffset']);
...
|| ####################################################################
|| #        SCRiPTMAFiA 2005 - THE DiRTY HANDS ON YOUR SCRiPTS
|| # CVS: $RCSfile: calendar.php,v $ - $Revision: 1.373 $
|| ####################################################################
\*======================================================================*/
?>