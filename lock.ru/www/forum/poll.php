...
if ($_REQUEST['do'] == 'showresults')
{
	...
	if ($pollinfo['public'])
	{
		...
		{
			$allnames["$name[voteoption]"][] = '<a href="member.php?' . $vbulletin->session->vars['sessionurl'] . "u=$name[userid]\">$name[username]</a>";
		}
	}
...
if ($_POST['do'] == 'pollvote')
{
	...
	if (!empty($vbulletin->GPC['optionnumber']))
	{
		...
		else if ($vbulletin->userinfo['userid'] AND $uservoteinfo = $db->query_first("
			SELECT userid
			FROM " . TABLE_PREFIX . "pollvote
...
|| ####################################################################
|| #        SCRiPTMAFiA 2005 - THE DiRTY HANDS ON YOUR SCRiPTS
|| # CVS: $RCSfile: poll.php,v $ - $Revision: 1.150 $
|| ####################################################################
\*======================================================================*/
?>