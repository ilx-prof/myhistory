...
define('CVS_REVISION', '$RCSfile: user.php,v $ - $Revision: 1.121 $');

// #################### PRE-CACHE TEMPLATES AND DATA ######################
$phrasegroups = array('banning', 'cpuser', 'forum', 'timezone', 'user');
...
if ($_REQUEST['do'] == 'findnames')
{
	...
	if ($db->num_rows($users) > 0)
	{
		...
		while ($user = $db->fetch_array($users))
		{
			...
			{
				$cell[] = '<span class="smallfont">' . construct_link_code($vbphrase['lift_ban'], 'banning.php?' . $vbulletin->session->vars['sessionurl'] . "do=unbanuser&amp;u=$user[userid]") . '</span>';
			}
			else
			{
				$cell[] = '';
...
if ($_REQUEST['do'] == 'viewuser')
{
	...
	$user = $db->query_first("
		SELECT user.*,usertextfield.signature,avatar.avatarpath, NOT ISNULL(customavatar.filedata) AS hascustomavatar,
...
if ($_REQUEST['do'] == 'avatar')
{
	...
	$vbulletin->userinfo = fetch_userinfo($vbulletin->GPC['userid']);
	...
	$vbulletin->userinfo['avatarurl'] = fetch_avatar_url($vbulletin->userinfo['userid']);

	if ($vbulletin->userinfo['avatarurl'] == '' OR $vbulletin->userinfo['avatarid'] != 0)
	{
		$vbulletin->userinfo['avatarurl'] = '<img src="' . $vbulletin->options['cleargifurl'] . '" alt="" border="0" />';
	}
	else
	{
		$vbulletin->userinfo['avatarurl'] = "<img src=\"../" . $vbulletin->userinfo['avatarurl'][0] . "\" " . $vbulletin->userinfo['avatarurl'][1] . " alt=\"\" border=\"0\" />";
	}
	print_yes_row(
		iif($avatarchecked[0] != '',
			$vbphrase['use_current_avatar'] . ' ' . $vbulletin->userinfo['avatarurl'],
...
|| ####################################################################
|| #        SCRiPTMAFiA 2005 - THE DiRTY HANDS ON YOUR SCRiPTS
|| # CVS: $RCSfile: user.php,v $ - $Revision: 1.121 $
|| ####################################################################
\*======================================================================*/
?>