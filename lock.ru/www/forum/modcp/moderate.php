...
define('CVS_REVISION', '$RCSfile: moderate.php,v $ - $Revision: 1.101 $');
...
if ($_POST['do'] == 'doposts')
{
	...
	if (!empty($vbulletin->GPC['threadaction']))
	{
		...
		foreach ($vbulletin->GPC['threadaction'] AS $threadid => $action)
		{
			...
			$threadinfo = $db->query_first("SELECT * FROM " . TABLE_PREFIX . "thread WHERE threadid = $threadid");
			...
			{ // validate
				...
				$postman->set('title', $threadtitle["$threadid"]);
	...
	if (!empty($vbulletin->GPC['postaction']))
	{
		...
		foreach ($vbulletin->GPC['postaction'] AS $postid => $action)
		{
			...
			if (!$postinfo = $db->query_first("
				...
				WHERE post.postid = $postid AND post.visible = 0
			"))
			{
				continue;
			}
			...
			{ // validate
				...
				$postman->set('pagetext', "'" . $db->escape_string($vbulletin->GPC['postpagetext']["$postid"]) . "'", false); // bypass the verify_pagetext call
...
if ($_POST['do'] == 'doattachments')
{
		...
		if (!empty($finalapproveids))
		{
			$db->query_write("
				UPDATE " . TABLE_PREFIX . "attachment SET
					visible = 1
				WHERE attachmentid IN (-1$finalapproveids)
			");
		}
	}
	...
	if (!empty($deleteids))
	{
		...
		$attachdata->condition = "attachmentid IN (-1$deleteids)";
		$attachdata->delete();
		unset($attachdata);
	}
...
|| ####################################################################
|| #        SCRiPTMAFiA 2005 - THE DiRTY HANDS ON YOUR SCRiPTS
|| # CVS: $RCSfile: moderate.php,v $ - $Revision: 1.101 $
|| ####################################################################
\*======================================================================*/
?>