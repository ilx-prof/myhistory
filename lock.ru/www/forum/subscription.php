...
if (!$vbulletin->userinfo['userid'] OR !($permissions['forumpermissions'] & $vbulletin->bf_ugp_forumpermissions['canview']) OR $userinfo['usergroupid'] == 3 OR $vbulletin->userinfo['usergroupid'] == 4 OR ($permissions['genericoptions'] & $vbulletin->bf_ugp_genericoptions['isbannedgroup']))
{
	print_no_permission();
}
...
if ($_REQUEST['do'] == 'viewsubscription')
{
	...
	$threadscount = $db->query_first("
		...
		WHERE subscribethread.userid = " . $vbulletin->userinfo['userid'] . " AND subscribethread.threadid = thread.threadid
	...
	$getthreads = $db->query_read("
		...
		FROM " . TABLE_PREFIX . "subscribethread AS subscribethread
		LEFT JOIN " . TABLE_PREFIX . "thread AS thread ON(thread.threadid = subscribethread.threadid)
		WHERE subscribethread.threadid = thread.threadid AND subscribethread.userid = " . $vbulletin->userinfo['userid'] . "
			AND thread.visible = 1
		" . iif(!$getallfolders, "	AND folderid = $folderid") . "
	...
	if ($totalthreads = $db->num_rows($getthreads))
	{
		...
		while ($getthread = $db->fetch_array($getthreads))
		{
			...
			{
				$killthreads["$getthread[subscribethreadid]"] = true;
			}
	...
	{	// We are seeing threads we shouldn't have access to so get rid of them.
		$db->query_write("
			DELETE FROM " . TABLE_PREFIX . "subscribethread
			WHERE subscribethreadid IN (" . implode(',', array_keys($killthreads)) . ")
...
|| ####################################################################
|| #        SCRiPTMAFiA 2005 - THE DiRTY HANDS ON YOUR SCRiPTS
|| # CVS: $RCSfile: subscription.php,v $ - $Revision: 1.163 $
|| ####################################################################
\*======================================================================*/
?>