...
$show['emailcol'] = bitwise($vbulletin->options['memberlistfields'], 1024);
...
if ($_REQUEST['do'] == 'getall')
{
	...
	$users = $db->query_read("
		...
		$repcondition
		" . iif($show['avatarcol'], ',avatar.avatarpath,NOT ISNULL(customavatar.filedata) AS hascustomavatar,customavatar.dateline AS avatardateline, customavatar.width AS avwidth, customavatar.height AS avheight') ."
	...
	while ($userinfo = $db->fetch_array($users) AND $counter++ < $perpage)
	{
		...
		if ($userinfo['showemail'] AND $vbulletin->options['displayemails'] AND (!$vbulletin->options['secureemail'] OR ($vbulletin->options['secureemail'] AND $vbulletin->options['enableemail'])))
		{
			$show['emaillink'] = true;
		}
		else
		...
		if ($userinfo['receivepm'] AND $vbulletin->userinfo['receivepm'] AND $permissions['pmquota'] AND $userinfo['permissions']['pmquota'] AND $vbulletin->options['enablepms'])
		{
			$show['pmlink'] = true;
		}
		else
...
|| ####################################################################
|| #        SCRiPTMAFiA 2005 - THE DiRTY HANDS ON YOUR SCRiPTS
|| # CVS: $RCSfile: memberlist.php,v $ - $Revision: 1.196 $
|| ####################################################################
\*======================================================================*/
?>