...
if ($_POST['do'] == 'processjoinrequests')
{
	...
	if (!($check = $db->query_first("SELECT * FROM " . TABLE_PREFIX . "usergroupleader WHERE userid = " . $vbulletin->userinfo['userid'] . " AND usergroupid = " . $vbulletin->GPC['usergroupid'])))
	{
		print_no_permission();
	}
...
|| ####################################################################
|| #        SCRiPTMAFiA 2005 - THE DiRTY HANDS ON YOUR SCRiPTS
|| # CVS: $RCSfile: joinrequests.php,v $ - $Revision: 1.51 $
|| ####################################################################
\*======================================================================*/
?>
