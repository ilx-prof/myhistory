...
// (depending on value of $cpnavjs)
	...
	global $_NAV, $_NAVPREFS, $nojs, $vbulletin, $vbphrase, $stylevar, $options, $groupid;
	...
	if (VB_AREA == 'AdminCP')
	{
		...
		<tr>
			...
			<td align=\"$stylevar[right]\">
				<a href=\"index.php?" . $vbulletin->session->vars['sessionurl'] . "do=buildnavprefs&amp;nojs=$nojs&amp;prefs=$navlinks&amp;dowhat=$dowhat&amp;id={$nav_file}_$groupid[$nav_file]#grp{$nav_file}_$groupid[$nav_file]\" target=\"_self\"
...
{
	global $_NAV, $_NAVPREFS, $groupid, $vbulletin, $vbphrase, $nojs;
	...
	if ($nojs)
	{
		$controls .= "<div align=\"center\">
			<a href=\"index.php?" . $vbulletin->session->vars['sessionurl'] . "do=navprefs&amp;nojs=$nojs&amp;groups=$groups&amp;expand=1&amp;$numgroups\" onclick=\"expand_all_groups(1); return false;\" target=\"_self\">" . $vbphrase['expand_all'] . "</a>
			|
			<a href=\"index.php?" . $vbulletin->session->vars['sessionurl'] . "do=navprefs&amp;nojs=$nojs&amp;groups=$groups&amp;expand=0&amp;$numgroups\" onclick=\"expand_all_groups(0); return false;\" target=\"_self\">" . $vbphrase['collapse_all'] . "</a>
		</div>";
	}
	else
	{
...
?>
	...
	function save_group_prefs(groupid)
	{
		...
		window.location = "index.php?<?php echo $vbulletin->session->vars['sessionurl_js']; ?>do=savenavprefs&nojs=<?php echo $nojs; ?>&navprefs=" + opengroups.join(",") + "#grp" + groupid;
	}
...
|| ####################################################################
|| #        SCRiPTMAFiA 2005 - THE DiRTY HANDS ON YOUR SCRiPTS
|| # CVS: $RCSfile: adminfunctions_navpanel.php,v $ - $Revision: 1.27 $
|| ####################################################################
\*======================================================================*/
?>