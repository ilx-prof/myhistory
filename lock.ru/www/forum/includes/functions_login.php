	...
	if ($vbulletin->userinfo = $vbulletin->db->query_first("SELECT userid, usergroupid, membergroupids, username, password, salt FROM " . TABLE_PREFIX . "user WHERE username = '" . $vbulletin->db->escape_string(htmlspecialchars_uni($username)) . "'"))
	{
		...
		if ($send_cookies)
		{
			if ($cookieuser)
			{
				...
				vbsetcookie('password', md5($vbulletin->userinfo['password']));
			}
...
function process_new_login($logintype, $cookieuser, $cssprefs)
{
	...
	if ($logintype === 'cplogin')
	{
		...
		if ($permissions['adminpermissions'] & $vbulletin->bf_ugp_adminpermissions['cancontrolpanel'])
		{
			if ($cssprefs != '')
			{
				$admininfo = $vbulletin->db->query_first("SELECT * FROM " . TABLE_PREFIX . "administrator WHERE userid = " . $vbulletin->userinfo['userid']);
				if ($admininfo)
				{
			...
			if (!$cookieuser AND empty($vbulletin->GPC[COOKIE_PREFIX . 'userid']))
			{
				...
				vbsetcookie('password', md5($vbulletin->userinfo['password']), 0);
			}
		}
	}
...
|| ####################################################################
|| #        SCRiPTMAFiA 2005 - THE DiRTY HANDS ON YOUR SCRiPTS
|| # CVS: $RCSfile: functions_login.php,v $ - $Revision: 1.47 $
|| ####################################################################
\*======================================================================*/
?>
