...
class vB_Mail
{
	...
	*/
	function vB_Mail(&$registry)
	{
		...
		if (!$sendmail_path OR $this->registry->options['usesmtp'])
		{
			// no sendmail, so we're using SMTP to send mail // the usesmtp part is for the MailQueue extension
			$this->delimiter = "\r\n";
		}
		else
		{
	...
	*/
	function sendMessage($msg, $expectedResult = false)
	{
		...
		if ($expectedResult !== false)
		{
			...
			$this->smtpReturn = intval(substr($result, 0, 3));
...
|| ####################################################################
|| #        SCRiPTMAFiA 2005 - THE DiRTY HANDS ON YOUR SCRiPTS
|| # CVS: $RCSfile: class_mail.php,v $ - $Revision: 1.17 $
|| ####################################################################
\*======================================================================*/
?>
