...
class vB_Postbit
{
				...
				if ($attachment['visible'])
				{
					switch($attachment['attachmentextension'])
					{
						...
						case 'pdf':
							...
							else if (!in_array($attachment['attachmentextension'], array('tiff', 'tif', 'psd', 'pdf')) AND ($this->registry->options['viewattachedimages'] == 2 OR ($this->registry->options['viewattachedimages'] == 1 AND $attachcount == 1)))
							{
								eval('$this->post[\'imageattachments\'] .= "' . fetch_template('postbit_attachmentimage') . '";');
								$show['imageattachment'] = true;
							}
	...
	*/
	function process_registered_user()
	{
		...
		if (empty($this->post['avatarurl']) OR ($this->registry->userinfo['userid'] > 0 AND !$this->registry->userinfo['showavatars']))
		{
			$show['avatar'] = false;
		}
		else
		...
		// get custom title
		if ($this->post['customtitle'] == 2)
		{
			// user title is not set by admin staff, so parse it.
			$this->post['usertitle'] = htmlspecialchars_uni($this->post['usertitle']);
		}
		...
		$show['pmlink'] = ($this->post['receivepm'] AND $this->registry->options['enablepms']);

		// Generate Age
		if ($this->registry->options['postelements'] & POST_SHOW_AGE AND $this->post['showbirthday'] > 0)
		{
			if (!$this->cache['year'])
			{
		...
		// get signature
		if ($this->post['showsignature'] AND $this->registry->options['allowsignatures'] AND trim($this->post['signature']) != ''
	...
	*/
	function prep_post_end()
	{
		...
		$show['reputationlink'] = (
			...
			AND !($this->registry->usergroupcache[$this->post['usergroupid']]['genericoptions'] & $this->registry->bf_ugp_genericoptions['isbannedgroup'])
		);
...
|| ####################################################################
|| #        SCRiPTMAFiA 2005 - THE DiRTY HANDS ON YOUR SCRiPTS
|| # CVS: $RCSfile: class_postbit.php,v $ - $Revision: 1.43 $
|| ####################################################################
\*======================================================================*/
?>