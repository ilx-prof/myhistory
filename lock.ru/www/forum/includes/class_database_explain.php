...
class vB_Database_Explain extends vB_Database
{
	...
	function db_connect($servername, $username, $password, $usepconnect)
	{
		...
		$return = parent::db_connect($servername, $username, $password, $usepconnect);
		$this->timer_stop();
		return $return;
	}
	...
	{
		if ($link == $this->connection_read AND preg_match('#(^|\s)SELECT\s#s', $this->sql))
		{
			$this->explain_query();
		}
		else
...
class vB_Database_MySQLi_Explain extends vB_Database_MySQLi
{
	...
	{
		$this->timer_start("Connect to Database on Server: <em>$servername</em>");
		$return = parent::db_connect($servername, $username, $password, $usepconnect, $configfile);
		$this->timer_stop();
		return $return;
	}
...
|| ####################################################################
|| #        SCRiPTMAFiA 2005 - THE DiRTY HANDS ON YOUR SCRiPTS
|| # CVS: $RCSfile: class_database_explain.php,v $ - $Revision: 1.10 $
|| ####################################################################
\*======================================================================*/
?>