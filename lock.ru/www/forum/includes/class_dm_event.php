...
class vB_DataManager_Event extends vB_DataManager
{
	...
	*/
	var $validfields = array(
		...
		'allowsmilies'  => array(TYPE_UINT,      REQ_YES,  VF_METHOD, 'verify_image_count'),
	...
	*/
	function verify_userid(&$userid)
	{
		...
		else if ($this->dbobject->query_first("SELECT userid FROM " . TABLE_PREFIX . "user WHERE userid = $userid"))
		{
			return true;
		}
		else
	...
	*/
	function verify_event(&$pagetext)
	{
		...
		if ($this->verify_image_count($pagetext, 'pagetext') == false)
		{
			return false;
		}
	...
	*/
	function verify_image_count(&$field, $type = 'smilie')
	{
		...
		{
			$allowsmilies =& $this->fetch_field('allowsmilies', 'post');
		}
		...
		if ($allowsmilies !== null AND $pagetext !== null)
		{
			// check max images
			require_once(DIR . '/includes/functions_misc.php');
			...
			{
				$imagecount = fetch_character_count($bbcode_parser->parse($pagetext, 'calendar', $allowsmilies, true), '<img');
	...
	*/
	function set_userfields(&$userfields)
	{
		...
		while ($custom = $this->dbobject->fetch_array($customcalfields))
		{
			...
			$optional    =& $userfields["o$custom[calendarcustomfieldid]"];
...
|| ####################################################################
|| #        SCRiPTMAFiA 2005 - THE DiRTY HANDS ON YOUR SCRiPTS
|| # CVS: $RCSfile: class_dm_event.php,v $ - $Revision: 1.13 $
|| ####################################################################
\*======================================================================*/
?>