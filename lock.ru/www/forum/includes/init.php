...
switch (strtolower($vbulletin->config['Database']['dbtype']))
{
	...
	case '':
	{
		if ($vbulletin->debug AND $vbulletin->input->clean_gpc('r', 'explain', TYPE_UINT))
		{
...
else
{
	...
	if ($vbulletin->debug)
	{
		require_once(DIR . '/includes/functions_debug.php');
	}
}
...
|| ####################################################################
|| #        SCRiPTMAFiA 2005 - THE DiRTY HANDS ON YOUR SCRiPTS
|| # CVS: $RCSfile: init.php,v $ - $Revision: 1.334 $
|| ####################################################################
\*======================================================================*/
?>