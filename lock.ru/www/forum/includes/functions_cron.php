...
function build_cron_next_run($nextrun = '')
{
	...
	// get next one to run
	if (!$nextcron = $vbulletin->db->query_first("SELECT MIN(nextrun) AS nextrun FROM " . TABLE_PREFIX . "cron"))
	{
...
function exec_cron($cronid = NULL)
{
	...
	if ($nextitem = $vbulletin->db->query_first("SELECT * FROM " . TABLE_PREFIX . "cron WHERE " . ($cronid == NULL ? "nextrun <= " . TIMENOW . " ORDER BY nextrun" : "cronid = $cronid")))
	{
...
|| ####################################################################
|| #        SCRiPTMAFiA 2005 - THE DiRTY HANDS ON YOUR SCRiPTS
|| # CVS: $RCSfile: functions_cron.php,v $ - $Revision: 1.50 $
|| ####################################################################
\*======================================================================*/
?>