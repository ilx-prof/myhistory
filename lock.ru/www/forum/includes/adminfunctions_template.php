...
* @return	string
*/
function fetch_template_update_sql($title, $template, $dostyleid, &$delete)
{
	...
	else
	{
		...
		return "
			...
			VALUES
				(" . intval($dostyleid) . ", '" . $vbulletin->db->escape_string($title) . "', '" . $vbulletin->db->escape_string($parsedtemplate) . "', '" . $vbulletin->db->escape_string($template) . "', 'template', " . TIMENOW . ", '" . $vbulletin->db->escape_string($vbulletin->userinfo['username']) . "', '" . $vbulletin->db->escape_string($vbulletin->options['templateversion']) . "')
		";
	}
...
function print_template_javascript()
{
	...
	echo '<script type="text/javascript"> var textarea_id = "' . $vbulletin->textarea_id . '"; </script>';
...
function xml_import_style($xml = false, $styleid = -1, $parentid = -1, $title = '', $anyversion = false, $displayorder = 1, $userselect = true)
{
	...
	$xmlobj = new XMLparser($xml, $vbulletin->GPC['path']);
...
|| ####################################################################
|| #        SCRiPTMAFiA 2005 - THE DiRTY HANDS ON YOUR SCRiPTS
|| # CVS: $RCSfile: adminfunctions_template.php,v $ - $Revision: 1.394 $
|| ####################################################################
\*======================================================================*/
?>