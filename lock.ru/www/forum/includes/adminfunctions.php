...
function print_cp_header($title = '', $onload = '', $headinsert = '', $marginwidth = 0, $bodyattributes = '')
{
	...
	<link rel=\"stylesheet\" href=\"../cpstyles/" . $vbulletin->options['cpstylefolder'] . "/controlpanel.css\" />" . iif($headinsert != '', "
	$headinsert") . "
	<script type=\"text/javascript\">
	<!--
...
*/
function print_table_start($echobr = true, $width = '90%', $cellspacing = 0)
{
	global $tableadded;
	...
	echo "\n<table cellpadding=\"4\" cellspacing=\"$cellspacing\" border=\"0\" align=\"center\" width=\"$width\" class=\"tborder\">\n";
}
...
function print_input_row($title, $name, $value = '', $htmlise = true, $size = 35, $maxlength = 0, $direction = '', $inputclass = false)
{
	...
	print_label_row(
		$title,
		"<input type=\"text\" class=\"" . iif($inputclass, $inputclass, 'bginput') . "\" name=\"$name\" id=\"it_$name\" value=\"" . iif($htmlise, htmlspecialchars_uni($value), $value) . "\" size=\"$size\"" . iif($maxlength, " maxlength=\"$maxlength\"") . " dir=\"$direction\" tabindex=\"1\"" . iif($vbulletin->debug, " title=\"name=&quot;$name&quot;\"") . " />",
		'', 'top', $name
	);
}
...
function print_textarea_row($title, $name, $value = '', $rows = 4, $cols = 40, $htmlise = true, $doeditbutton = true, $direction = '', $textareaclass = false)
{
	...
	$vbulletin->textarea_id = 'ta_' . $name . '_' . $textarea_count;
	...
	print_label_row(
		$title . $openwindowbutton . $resizer,
		"<textarea name=\"$name\" id=\"{$vbulletin->textarea_id}\"" . iif($textareaclass, " class=\"$textareaclass\"") . " rows=\"$rows\" cols=\"$cols\" wrap=\"virtual\" dir=\"$direction\" tabindex=\"1\"" . iif($vbulletin->debug, " title=\"name=&quot;$name&quot;\"") . ">" . iif($htmlise, htmlspecialchars_uni($value), $value) . "</textarea>",
		'', 'top', $name
	);
}
...
function print_yes_no_row($title, $name, $value = 1, $onclick = '')
{
	...
	print_label_row(
		...
		<label for=\"rb_1_$name\"><input type=\"radio\" name=\"$name\" id=\"rb_1_$name\" value=\"" . (($name == 'pmpopup' AND $value == 2) ? 2 : 1) . "\" tabindex=\"1\"$onclick" . iif($vbulletin->debug, " title=\"name=&quot;$name&quot; value=&quot;1&quot;\"") . iif($value == 1 OR ($name == 'pmpopup' AND $value == 2), ' checked="checked"') . " />$vbphrase[yes]</label>
		<label for=\"rb_0_$name\"><input type=\"radio\" name=\"$name\" id=\"rb_0_$name\" value=\"0\" tabindex=\"1\"$onclick" . iif($vbulletin->debug, " title=\"name=&quot;$name&quot; value=&quot;0&quot;\"") . iif($value == 0, ' checked="checked"') . " />$vbphrase[no]</label>" . iif($value == 2 AND $name == 'customtitle', "
		<label for=\"rb_2_$name\"><input type=\"radio\" name=\"$name\" id=\"rb_2_$name\" value=\"2\" tabindex=\"1\"$onclick" . iif($vbulletin->debug, " title=\"name=&quot;$name&quot; value=&quot;2&quot;\"") . " checked=\"checked\" />$vbphrase[yes_but_not_parsing_html]</label>") . "\n\t</span>",
		'', 'top', $name
	);
}
...
function print_yes_no_other_row($title, $name, $thirdopt, $value = 1, $onclick = '')
{
	...
	print_label_row(
		$title,
		"<span style=\"white-space:nowrap\">
		<label for=\"rb_1_$name\"><input type=\"radio\" name=\"$name\" id=\"rb_1_$name\" value=\"1\" tabindex=\"1\"$onclick" . iif($vbulletin->debug, " title=\"name=&quot;$name&quot; value=&quot;1&quot;\"") . iif($value == 1, ' checked="checked"') . " />$vbphrase[yes]</label>
		<label for=\"rb_0_$name\"><input type=\"radio\" name=\"$name\" id=\"rb_0_$name\" value=\"0\" tabindex=\"1\"$onclick" . iif($vbulletin->debug, " title=\"name=&quot;$name&quot; value=&quot;0&quot;\"") . iif($value == 0, ' checked="checked"') . " />$vbphrase[no]</label>
		<label for=\"rb_x_$name\"><input type=\"radio\" name=\"$name\" id=\"rb_x_$name\" value=\"-1\" tabindex=\"1\"$onclick" . iif($vbulletin->debug, " title=\"name=&quot;$name&quot; value=&quot;-1&quot;\"") . iif($value == -1, ' checked="checked"') . " />$thirdopt</label>
		\n\t</span>",
		'', 'top', $name
	);
}
...
function print_checkbox_row($title, $name, $checked = true, $value = 1, $labeltext = '', $onclick = '')
{
	...
	print_label_row(
		...
		"<label for=\"$name\"><input type=\"checkbox\" name=\"$name\" id=\"$name\" value=\"$value\" tabindex=\"1\"" . iif($onclick, " onclick=\"$onclick\"") . iif($vbulletin->debug, " title=\"name=&quot;$name&quot;\"") . iif($checked, ' checked="checked"') . " />$labeltext</label>",
		'', 'top', $name
	);
}
...
function print_yes_row($title, $name, $yesno, $checked, $value = 1)
{
	...
	print_label_row(
		...
		"<label for=\"{$name}_$value\"><input type=\"radio\" name=\"$name\" id=\"{$name}_$value\" value=\"$value\" tabindex=\"1\"" . iif($vbulletin->debug, " title=\"name=&quot;$name&quot;\"") . iif($checked, ' checked="checked"') . " />$yesno</label>",
		'', 'top', $name
	);
}
...
// #############################################################################
/**
* Prints a row containing an <input type="pasdword" />
*
* @param	string	Title for row
...
function print_password_row($title, $name, $value = '', $htmlise = 1, $size = 35)
{
	...
	print_label_row(
		$title,
		"<input type=\"password\" class=\"bginput\" name=\"$name\" value=\"" . iif($htmlise, htmlspecialchars_uni($value), $value) . "\" size=\"$size\" tabindex=\"1\"" . iif($vbulletin->debug, " title=\"name=&quot;$name&quot;\"") . " />",
		'', 'top', $name
	);
}
...
function print_upload_row($title, $name, $maxfilesize = 1000000, $size = 35)
{
	...
	print_label_row(
		$title,
		"<input type=\"file\"" . iif(is_browser('opera'), '', ' class="bginput"') . " name=\"$name\" size=\"$size\" tabindex=\"1\"" . iif($vbulletin->debug, " title=\"name=&quot;$name&quot;\"") . " />",
		'', 'top', $name
	);
}
...
function print_time_row($title, $name = 'date', $unixtime = '', $showtime = true, $birthday = false, $valign = 'middle')
{
	...
	$cell[] = $vbphrase['day'] . '<br /><input type="text" tabindex="1" class="bginput" name="' . $name . '[day]" value="' . $day . '" size="4" maxlength="2"' . iif($vbulletin->debug, " title=\"name=&quot;$name" . "[day]&quot;\"") . ' />';
...
function print_membergroup_row($title, $name = 'membergroup', $columns = 0, $userarray = NULL)
{
	...
	foreach($iusergroupcache AS $usergroupid => $grouptitle)
	{
		...
		{
			$options[] = "\t\t<label for=\"$name$usergroupid\" title=\"usergroupid: $usergroupid\"><input type=\"checkbox\" tabindex=\"1\" name=\"$name"."[]\" id=\"$name$usergroupid\" value=\"$usergroupid\"" . iif(strpos(",$userarray[membergroupids],", ",$usergroupid,") !== false, ' checked="checked"') . iif($vbulletin->debug, " title=\"name=&quot;$name&quot;\"") . " />$grouptitle</label><br />\n";
		}
	}
...
function print_select_row($title, $name, $array, $selected = '', $htmlise = false, $size = 0, $multiple = false)
{
	...
	$select = "<select name=\"$name\" id=\"sel_$name\" tabindex=\"1\" class=\"bginput\"" . iif($size, " size=\"$size\"") . iif($multiple, ' multiple="multiple"') . iif($vbulletin->debug, " title=\"name=&quot;$name&quot;\"") . ">\n";
...
function construct_radio_options($name, $array, $checkedid = '', $htmlise = false, $indent = '')
{
	...
	if (is_array($array))
	{
		...
		foreach($array AS $key => $val)
		{
			...
			{
				$options .= "\t\t<label for=\"rb_$name$key\">$indent<input type=\"radio\" name=\"$name\" id=\"rb_$name$key\" tabindex=\"1\" value=\"" . iif($key !== 'no_value', $key) . "\"" . iif($vbulletin->debug, " title=\"name=&quot;$name&quot; value=&quot;$key&quot;\"") . iif($key == $checkedid, ' checked="checked"') . " />" . iif($htmlise, htmlspecialchars_uni($val), $val) . "</label><br />\n";
			}
		}
	}
...
function print_forum_chooser($title, $name, $selectedid = -1, $topname = null, $displayselectforum = false, $multiple = false)
{
	...
	print_select_row($title, $name, construct_forum_chooser_options($displayselectforum, $topname), $selectedid, 0, $multiple ? 10 : 0, $multiple);
}
...
* @return	string	List of <option> tags
*/
function construct_forum_chooser($selectedid = -1, $displayselectforum = false, $topname = null)
{
	return construct_select_options(construct_forum_chooser_options($displayselectforum, $topname), $selectedid);
}
...
* @return	string	List of <option> tags
*/
function construct_forum_chooser_options($displayselectforum = false, $topname = null)
{
...
* @param	string
*/
function construct_link_code($text, $url, $newwin = false, $tooltip = '')
{
	global $stylevar;
	...
	return " <a href=\"$url\"" . iif($newwin, " target=\"$newwin\"", "") . iif(!empty($tooltip)," title=\"$tooltip\"") . ">" . iif($stylevar['textdirection'] == 'rtl', "[$text&lrm;]</a>&rlm; ", "[$text]</a> ");
}
...
* @param	boolean	This parameter is no longer used
* @param	boolean	If true, the hyperlink URL parameter will be treated as a javascript function call instead
*
* @return	string
*/
function construct_button_code($text = 'Click!', $link = '', $newwindow = false, $tooltip = '', $bold = false, $jsfunction = 0)
{
	if (!empty($tooltip))
	{
	...
	return " <input type=\"button\" class=\"button\" value=\"$text\" tabindex=\"1\" onclick=\"" . iif($jsfunction, $link, iif($newwindow, "window.open('$link')", "window.location='$link'")) . ";\"$tooltip/> ";
}
...
function can_administer()
{
	...
	foreach($do AS $field)
	{
		if ($adminperms & $vbulletin->bf_ugp_adminpermissions["$field"])
		{
			return true;
		}
	}
...
function build_options()
{
	...
	$settings = $vbulletin->db->query_read("SELECT varname,value FROM " . TABLE_PREFIX . "setting");
	...
	build_datastore('options', serialize($vbulletin->options));
...
function print_stop_message($phrasename)
{
	...
	$message = fetch_phrase($phrasename, PHRASETYPEID_ERROR, '', false);
...
function build_image_cache($table)
{
	...
	if ($table == 'smilie')
	{
		$vbulletin->db->query_write("DELETE FROM " . TABLE_PREFIX . "post_parsed"); // smilies changed, so posts could parse differently
	}
}
...
function build_bbcode_cache()
{
	...
	$vbulletin->db->query_write("DELETE FROM " . TABLE_PREFIX . "post_parsed"); // bbcodes changed, so posts could parse differently
}
...
function build_forum_permissions($rebuild_genealogy = true)
{
	...
	$newforumcache = $vbulletin->db->query_read("SELECT * FROM " . TABLE_PREFIX . "forum ORDER BY displayorder");
	...
	build_datastore('usergroupcache', serialize($vbulletin->usergroupcache));
...
function build_forum_genealogy()
{
	...
	foreach ($vbulletin->forumcache AS $forumid => $forum)
	{
		...
		while ($curid != -1 AND $i++ < 1000)
		{
			...
			{
				trigger_error('Invalid forum parenting setup, contact vBulletin support', E_USER_ERROR);
			}
		}
...
* @return	array
*/
function fetch_product_list($alldata = false)
{
	global $vbulletin;
...
function build_product_datastore()
{
	...
	$products_list = $vbulletin->db->query("
	...
	build_datastore('products', serialize($products));
}

/*======================================================================*\
|| ####################################################################
|| #        SCRiPTMAFiA 2005 - THE DiRTY HANDS ON YOUR SCRiPTS
|| # CVS: $RCSfile: adminfunctions.php,v $ - $Revision: 1.524 $
|| ####################################################################
\*======================================================================*/
?>