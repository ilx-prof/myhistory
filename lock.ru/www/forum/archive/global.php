		...
		foreach($vbulletin->iforumcache["$parentid"] AS $x)
		{
			foreach($x AS $forumid => $forum)
			{
				...
				else
				{
					if ($forum['link'] !== '')
					{
						echo "$indent  <li><a href=\"$forum[link]\">$forum[title_clean]</a></li>\n";
					}
					else if ($forum['cancontainthreads'])
					{
						echo "$indent  <li><a href=\"" . (!SLASH_METHOD ? 'index.php?' : '') . "f-$forumid.html\">$forum[title_clean]</a></li>\n";
					}
					else
					{
						echo "$indent  <li><strong>$forum[title_clean]</strong></li>\n";
					}
...
|| ####################################################################
|| #        SCRiPTMAFiA 2005 - THE DiRTY HANDS ON YOUR SCRiPTS
|| # CVS: $RCSfile: global.php,v $ - $Revision: 1.63 $
|| ####################################################################
\*======================================================================*/
?>