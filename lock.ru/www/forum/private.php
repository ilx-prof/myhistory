...
if ($permissions['pmquota'] < 1 OR !$vbulletin->userinfo['userid'])
{
	print_no_permission();
}
...
{ // confirmation page
	...
	$vbulletin->input->clean_gpc('p', 'folderid', TYPE_INT);
...
if ($_REQUEST['do'] == 'downloadpm')
{
	...
	switch ($vbulletin->GPC['dowhat'])
	{
		...
		case 'xml':
			...
			// download the file
			file_download($doc, "$vbphrase[dump_privatemessages]-" . $vbulletin->userinfo['username'] . "-" . vbdate($vbulletin->options['dateformat'], TIMENOW) . '.xml', 'text/xml');
		break;
		...
		case 'csv':
			...
			// download the file
			file_download($csv, "$vbphrase[dump_privatemessages]-" . $vbulletin->userinfo['username'] . "-" . vbdate($vbulletin->options['dateformat'], TIMENOW) . '.csv', 'text/x-csv');
		break;
		...
		case 'txt':
			...
			// download the file
			file_download($txt, "$vbphrase[dump_privatemessages]-" . $vbulletin->userinfo['username'] . "-" . vbdate($vbulletin->options['dateformat'], TIMENOW) . '.txt', 'text/plain');
		break;
	...
	else
	{
		...
		else
		{
			...
			if ($vbulletin->GPC['userid'])
			{
				...
				$users = $db->query_read("SELECT username FROM " . TABLE_PREFIX . "user AS user WHERE userid IN(" . implode(', ', $recipients) . ")");
				$recipients = array();
...
if ($_REQUEST['do'] == 'showpm')
{
	...
	if ($pm['messageread'] == 0)
	{
		...
		$userdm->set('pmunread', 'pmunread - 1', false);
...
|| ####################################################################
|| #        SCRiPTMAFiA 2005 - THE DiRTY HANDS ON YOUR SCRiPTS
|| # CVS: $RCSfile: private.php,v $ - $Revision: 1.315 $
|| ####################################################################
\*======================================================================*/
?>