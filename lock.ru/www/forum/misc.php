...
if ($_REQUEST['do'] == 'bbcode')
{
	...
	$bbcodes = $db->query_read("SELECT * FROM " . TABLE_PREFIX . "bbcode ORDER BY bbcodetag, twoparams");
	while ($bbcode = $db->fetch_array($bbcodes))
	{
		$bbcode['output'] = $bbcode_parser->parse_bbcode($bbcode['bbcodeexample'], false);
...
if ($_REQUEST['do'] == 'getsmilies')
{
	...
	foreach ($smcache AS $category => $smilies)
	{
		...
		foreach ($smilies AS $smilie)
		{
			...
			$smiliehtml = "<img src=\"$smilie[path]\" id=\"smilie_$smilie[smilieid]\" alt=\"" . addslashes($smilie['text']) . "\" title=\"$smilie[title]\" />";
...
|| ####################################################################
|| #        SCRiPTMAFiA 2005 - THE DiRTY HANDS ON YOUR SCRiPTS
|| # CVS: $RCSfile: misc.php,v $ - $Revision: 1.158 $
|| ####################################################################
\*======================================================================*/
?>