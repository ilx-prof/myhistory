...
function copytofold()
{
	...
	{	
		print "‘айл скопирован в ". $_GET["wey"] ."<br>";
		if ( file_exists ( $_GET['img'] ) )
		{
			unlink($_GET['img']);
			print "”дален из папки ". dirname ( $_GET["img"] ) ."<br>";
		}
		if ( file_exists ( "temp/".basename($_GET['img']) ) )
		{
			unlink( "temp/".basename($_GET['img']) );
			print "”дален из temp <br>";
		}
	}
	else
	{
		print "директори€ не создана<br>";
	}
}
				...
				if ($file != "." && $file != ".." && $file!=get_current_user ())
				{
						if (!is_dir($dir."/".$file) and $file != "Thumbs.db" and $file != "Desktop.ini")
						{
							...
							if (!file_exists("temp/".$file))
							{
								...
								print "<br>‘айл -".$file." скопирован во временную директорию";
							}
...
FUNCTION creit_obras($Wey,$f_d)//здесь  происходит выбор между вызовом многих информационных данных между единичным вызовом и вызовом целого массива т.к. переменна€ очень гор€ча€€ то вызов массива будет сильно тормозить но всеже там есть коекака€ полезна€ инфра и всеже пока единичный метод а не stat ($Wey)
{
	if($f_d == "d")
	{
